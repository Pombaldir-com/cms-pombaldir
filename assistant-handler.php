...
function extractAccountsFromPayload(string $json): array {
    ...
    foreach ($source as $key => $value) {
        ...
        $rubricCode = normalizeAccountingRubricCodeValue($value['erp_rubric_code'] ?? ($value['rubric_code'] ?? ''));
        if ($iva === '' && $general === '' && $rubricCode === '') {
            continue;
        }
        $rates[$rateKey] = [
            'iva_account' => $iva,
        ...
        if ($rubricCode !== '') {
            $rates[$rateKey]['erp_rubric_code'] = $rubricCode;
        }
    }
    return $rates;
}
...
function buildSuggestedAccounts(array $examples): array {
    ...
    foreach ($examples as $example) {
        ...
        foreach ($rates as $rateKey => $accounts) {
            ...
            $rubricCode = normalizeAccountingRubricCodeValue($accounts['erp_rubric_code'] ?? '');
            if ($iva !== '') {
            ...
            if ($rubricCode !== '') {
                $tally[$rateKey]['rubric'][$rubricCode] = ($tally[$rateKey]['rubric'][$rubricCode] ?? 0) + 1;
            }
        }
    }
    $suggested = [];
    foreach ($tally as $rateKey => $counts) {
        ...
        if (!empty($counts['rubric'])) {
            arsort($counts['rubric']);
            $rubricCode = (string) array_key_first($counts['rubric']);
        }
        if ($iva !== '' || $general !== '' || $rubricCode !== '') {
            $suggested[$rateKey] = [
                'iva_account' => $iva,
            ...
            if ($rubricCode !== '') {
                $suggested[$rateKey]['erp_rubric_code'] = $rubricCode;
            }
        }
    }
    return $suggested;
...
function resolveAccountByRateKey(array $suggested, string $rateKey): array {
    ...
    return ['iva_account' => '', 'general_account' => '', 'erp_rubric_code' => ''];
}
...
function mergeSuggestedAccounts(array $primary, array $fallback): array {
    ...
    foreach ($fallback as $rate => $accounts) {
        ...
        if (($result[$rate]['erp_rubric_code'] ?? '') === '' && ($accounts['erp_rubric_code'] ?? '') !== '') {
            $result[$rate]['erp_rubric_code'] = $accounts['erp_rubric_code'];
        }
    }
    return $result;
}
...
function getErpDatabaseForNif(string $nif): ?string {
    ...
    $pdo = getPDO();
    $stmt = $pdo->prepare('SELECT erp_database, entity_type, erp_client_code FROM accounting_entities WHERE nif = ? LIMIT 1');
    $stmt->execute([$nif]);
...
function getErpDefaultDb(): string {
...
function hasStoredClassificationSuggestionForContext(string $docType, string $emitter, string $acquirer, array $context = []): bool {
    if (!hasTable('accounting_classifications')) {
        return false;
    }
    ...
    $emitterCandidates = array_values(array_unique(array_filter([
        extractVatLikeValue((string) ($context['emitter_nif'] ?? '')),
        extractVatLikeValue((string) ($context['emitter_raw'] ?? '')),
        extractVatLikeValue($emitter),
        trim((string) ($context['emitter_raw'] ?? '')),
        trim($emitter),
    ], static function ($value): bool {
        return is_string($value) && trim($value) !== '';
    })));
    $acquirerCandidates = array_values(array_unique(array_filter([
        extractVatLikeValue((string) ($context['acquirer_nif'] ?? '')),
        extractVatLikeValue((string) ($context['acquirer_raw'] ?? '')),
        extractVatLikeValue($acquirer),
        trim((string) ($context['acquirer_raw'] ?? '')),
        trim($acquirer),
    ], static function ($value): bool {
        return is_string($value) && trim($value) !== '';
    })));
    if (!$emitterCandidates || !$acquirerCandidates) {
        return false;
    }
    ...
    foreach ($emitterCandidates as $emitterCandidate) {
        foreach ($acquirerCandidates as $acquirerCandidate) {
            foreach ($docTypeCandidates as $docTypeCandidate) {
                $stmt->execute([$emitterCandidate, $acquirerCandidate, $docTypeCandidate]);
                $accountPayload = $stmt->fetchColumn();
                if (!is_string($accountPayload) || trim($accountPayload) === '') {
                    continue;
                }
                $rates = extractAccountsFromPayload($accountPayload);
                $totalAccount = extractTotalAccountFromPayload($accountPayload, normalizeSuggestionReceiptCompanionFlag($context['has_receipt_companion'] ?? '0'));
                if ($rates || trim($totalAccount) !== '') {
                    return true;
                }
            }
        }
...
function normalizeAiInstructionComparable(string $value): string {
    $value = strtr($value, [
        'Á' => 'A', 'À' => 'A', 'Â' => 'A', 'Ã' => 'A', 'Ä' => 'A',
        'á' => 'a', 'à' => 'a', 'â' => 'a', 'ã' => 'a', 'ä' => 'a',
        'É' => 'E', 'È' => 'E', 'Ê' => 'E', 'Ë' => 'E',
...
function extractAiAccountingInstructionSection(): string {
...
function extractAiInstructionAccountCode(string $line, array $labelPatterns): string {
    if (!$labelPatterns) {
        return '';
    }
    $pattern = '/(?:' . implode('|', $labelPatterns) . ')\s*(?:=|:|->|-)?\s*([0-9][0-9 .\/-]{2,})/iu';
    if (preg_match($pattern, $line, $match)) {
        return cleanAiInstructionAccountCode((string) ($match[1] ?? ''));
    }
    return '';
}

function extractAiInstructionAccountValue(string $line, array $labelPatterns): ?string {
    if (!$labelPatterns) {
        return null;
    }
    ...
    if (strpos(normalizeAiInstructionComparable($rawValue), 'vazio') !== false) {
        return '';
    }
    $account = cleanAiInstructionAccountCode($rawValue);
    return $account !== '' ? $account : null;
}

function splitAiInstructionAccountClauses(string $line): array {
...
function aiInstructionLineMatchesContext(string $line, array $context): bool {
    $contextNifs = array_values(array_unique(array_filter([
        extractVatLikeValue((string) ($context['emitter_nif'] ?? '')),
        extractVatLikeValue((string) ($context['emitter'] ?? '')),
        extractVatLikeValue((string) ($context['emitter_raw'] ?? '')),
        extractVatLikeValue((string) ($context['acquirer_nif'] ?? '')),
        extractVatLikeValue((string) ($context['acquirer_raw'] ?? '')),
    ], static function ($value): bool {
    ...
    if (preg_match_all('/["“”]([^"“”]{2,120})["“”]/u', $line, $nameMatches)) {
        $contextNames = array_values(array_filter([
            normalizeAiInstructionComparable((string) ($context['emitter'] ?? '')),
            normalizeAiInstructionComparable((string) ($context['emitter_raw'] ?? '')),
            normalizeAiInstructionComparable((string) ($context['acquirer_raw'] ?? '')),
        ], static function ($value): bool {
            return is_string($value) && $value !== '';
        }));
        foreach ($nameMatches[1] as $quotedName) {
            $normalizedName = normalizeAiInstructionComparable((string) $quotedName);
            if ($normalizedName === '') {
                continue;
            }
            foreach ($contextNames as $contextName) {
                if (strpos($contextName, $normalizedName) !== false || strpos($normalizedName, $contextName) !== false) {
                    return true;
                }
            }
        }
...
function resolveAiInstructionTargetRateKey(string $line, array $rateItems): string {
    ...
    if ((strpos($normalizedLine, 'comissao') !== false || strpos($normalizedLine, 'imposto selo') !== false || strpos($normalizedLine, 'selo') !== false)
    ...
    foreach ($rateItems as $rateInfo) {
        ...
        if ($normalizedRateKey !== '' && strpos($normalizedLine, $normalizedRateKey . ' por cento') !== false) {
            return $rateKey;
        }
    }
...
function resolveLigacaoRateKeyFromRow(array $row): string {
    ...
    if ($rateValue !== '' && $rateValue !== '.000000' && $rateValue !== '0.000000' && $rateValue !== '0') {
        return normalizeRateKey($rateValue);
    }

    // In LigacaoCteTipoDoc, fltTaxaValor/fallback numeric fields often come as 0.
    // The reliable discriminator is usually PC_Descricao:
    // TAXA REDUZIDA => 6, TAXA INTERMEDIA => 13, TAXA NORMAL => 23.
...
function resolveErpLigacaoRubricCode(array $row): string {
    return normalizeAccountingRubricCodeValue($row['Rub_Codigo'] ?? '');
}
...
function fetchErpLigacaoAccountHints(string $baseUrl, string $token, string $db, string $docType, string $acquirerNif, string $docDate): array {
    ...
    foreach ($rows as $row) {
        ...
        if ($rateKey !== '' && !isset($perRate[$rateKey])) {
            $perRate[$rateKey] = ['general' => [], 'iva' => [], 'rubric_code' => []];
        }
        $rubricCode = resolveErpLigacaoRubricCode($row);
        if ($general !== '') {
            $generalCounts[$general] = ($generalCounts[$general] ?? 0) + 1;
            if ($rateKey !== '') {
                $perRate[$rateKey]['general'][$general] = ($perRate[$rateKey]['general'][$general] ?? 0) + 1;
            }
        }
        if ($iva !== '') {
            $ivaCounts[$iva] = ($ivaCounts[$iva] ?? 0) + 1;
            if ($rateKey !== '') {
                $perRate[$rateKey]['iva'][$iva] = ($perRate[$rateKey]['iva'][$iva] ?? 0) + 1;
            }
        }
        if ($rubricCode !== '') {
            $rubricCodeCounts[$rubricCode] = ($rubricCodeCounts[$rubricCode] ?? 0) + 1;
            if ($rateKey !== '') {
                $perRate[$rateKey]['rubric_code'][$rubricCode] = ($perRate[$rateKey]['rubric_code'][$rubricCode] ?? 0) + 1;
            }
        }
    ...
    foreach ($perRate as $rateKey => $entry) {
        ...
        arsort($entry['iva']);
        arsort($entry['rubric_code']);
        $perRate[$rateKey] = [
            'general' => array_slice(array_keys($entry['general']), 0, 8),
            'iva' => array_slice(array_keys($entry['iva']), 0, 8),
            'rubric_code' => array_slice(array_keys($entry['rubric_code']), 0, 8),
        ];
    }
...
function findPlanAccountByCode(array $planAccounts, string $accountCode, string $field = 'account'): ?array {
    $target = trim($accountCode);
    if ($target === '' || !$planAccounts) {
        return null;
    }
    foreach ($planAccounts as $row) {
        if (!is_array($row)) {
...
function resolveTargetRateValue(array $rateInfo): ?float {
    $label = $rateInfo['label'] ?? '';
    $rateKey = $rateInfo['key'] ?? '';
    $normalized = trim(str_replace('%', '', $label ?: $rateKey));
    if ($normalized === '') {
    ...
    $target = (float) str_replace(',', '.', $normalized);
...
function accountMatchesRateInfo(array $planAccounts, string $accountCode, array $rateInfo, string $field = 'account'): bool {
    $targetRate = resolveTargetRateValue($rateInfo);
    if ($targetRate === null) {
        return true;
    }
    $row = findPlanAccountByCode($planAccounts, $accountCode, $field);
    if ($row === null) {
        return false;
    }
    $vatRate = trim((string) ($row['tax_rate'] ?? ''));
    if ($vatRate !== '') {
        $value = (float) str_replace(',', '.', $vatRate);
        return abs($value - $targetRate) < 0.001;
    }
    $description = strtolower(trim((string) ($row['description'] ?? '')));
    if ($description !== '') {
        if ($targetRate === 6.0) {
            return strpos($description, 'reduz') !== false;
        }
        if ($targetRate === 13.0) {
            return strpos($description, 'interm') !== false;
        }
        if ($targetRate === 23.0) {
            return strpos($description, 'normal') !== false;
        }
    }
    return false;
}

function pickMatchingAccountForRate(array $candidates, array $planAccounts, array $rateInfo, string $field = 'account'): string {
    if (!$planAccounts) {
        foreach ($candidates as $candidate) {
    ...
    foreach ($candidates as $candidate) {
        ...
        if ($firstCandidate === '') {
            $firstCandidate = $account;
        }
        if (findPlanAccountByCode($planAccounts, $account, $field) !== null) {
            $foundInPlan = true;
        }
        if (accountMatchesRateInfo($planAccounts, $account, $rateInfo, $field)) {
            return $account;
        }
    }
    if (!$foundInPlan) {
...
function runSuggestAccounts(array $args, bool $canSuggestVat, string $erpBaseUrl, string $erpToken, string $openAiKey = '', string $openAiModel = ''): array {
    ...
    if ($erpBaseUrl !== '' && $erpToken !== '') {
        ...
        if (!empty($ligacaoNifCandidates)) {
            $primaryLigacaoNif = (string) $ligacaoNifCandidates[0];
        }
        $resolvedByLigacaoNif = $primaryLigacaoNif !== '' ? (getErpDatabaseForNif($primaryLigacaoNif) ?? '') : '';
        $resolvedByAcquirer = getErpDatabaseForNif($acquirerNif) ?? '';
        $defaultDb = getErpDefaultDb();
        if ($requestedDb !== '') {
            $planDb = $requestedDb;
        ...
        if ($planDb !== '') {
            ...
            foreach ($dbCandidates as $dbCandidate) {
                $nifCandidatesForDb = $ligacaoNifCandidates;
                if ($primaryLigacaoNif !== '') {
                    $nifCandidatesForDb = array_values(array_unique(array_filter(array_merge([$primaryLigacaoNif], $ligacaoNifCandidates), static function ($value): bool {
                        return is_string($value) && trim($value) !== '';
                    })));
                }
            ...
            if ($ligacaoNifUsed === '' && !empty($ligacaoNifCandidates)) {
                $ligacaoNifUsed = $primaryLigacaoNif !== '' ? $primaryLigacaoNif : (string) $ligacaoNifCandidates[0];
            }
    ...
    if (!empty($ligacaoHints['general']) || !empty($ligacaoHints['iva'])) {
        ...
        $ligacaoPerRate = is_array($ligacaoHints['per_rate'] ?? null) ? $ligacaoHints['per_rate'] : [];
        $ligacaoRubricCodes = is_array($ligacaoHints['rubric_codes'] ?? null) ? $ligacaoHints['rubric_codes'] : [];
        foreach ($rateItems as $rateInfo) {
            $rateKey = (string) ($rateInfo['key'] ?? '');
            if ($rateKey === '') {
                continue;
            ...
            $normalizedRateKey = normalizeRateKey($rateKey);
            $perRateEntry = $ligacaoPerRate[$normalizedRateKey] ?? $ligacaoPerRate[$rateKey] ?? [];
            $perRateGeneral = is_array($perRateEntry['general'] ?? null) ? $perRateEntry['general'] : [];
            $perRateIva = is_array($perRateEntry['iva'] ?? null) ? $perRateEntry['iva'] : [];
            $perRateRubricCodes = is_array($perRateEntry['rubric_code'] ?? null) ? $perRateEntry['rubric_code'] : [];
            ...
            $rubricCodeCandidates = !empty($perRateRubricCodes)
                ? $perRateRubricCodes
                : ($trustUnscopedLigacaoCandidates ? $ligacaoRubricCodes : []);
            if (!empty($rubricCodeCandidates)) {
                $matchedRubricCode = trim((string) pickFirstAccountCandidate($rubricCodeCandidates));
                if ($matchedRubricCode !== '') {
                    $finalSuggested[$rateKey]['erp_rubric_code'] = $matchedRubricCode;
                }
            }
            if ($normalizedRateKey !== '0' && ($finalSuggested[$rateKey]['iva_account'] ?? '') === '' && $planAccounts) {
                ...
                if ($generalForRate !== '') {
                    ...
                    if ($fallbackIva !== '' && accountMatchesRateInfo($planAccounts, $fallbackIva, $rateInfo, 'iva_account')) {
                        $finalSuggested[$rateKey]['iva_account'] = $fallbackIva;
                    }
                }
                if (($finalSuggested[$rateKey]['iva_account'] ?? '') === '') {
                    $fallbackIva = findIvaAccountForRate($planAccounts, $rateInfo);
                    if ($fallbackIva !== '') {
                        $finalSuggested[$rateKey]['iva_account'] = $fallbackIva;
                    }
                }
    ...
    $aiInstructionRates = is_array($aiInstructionSuggestions['rates'] ?? null) ? $aiInstructionSuggestions['rates'] : [];
    if ($aiInstructionRates) {
        foreach ($aiInstructionRates as $rateKey => $accounts) {
            $rateKey = (string) $rateKey;
            if ($rateKey === '' || !is_array($accounts)) {
                continue;
            }
            if (!isset($finalSuggested[$rateKey])) {
                $finalSuggested[$rateKey] = ['iva_account' => '', 'general_account' => ''];
            ...
            $general = trim((string) ($accounts['general_account'] ?? ''));
            $iva = trim((string) ($accounts['iva_account'] ?? ''));
            $hasGeneralInstruction = !empty($accounts['general_account_explicit']);
            $hasIvaInstruction = !empty($accounts['iva_account_explicit']);
            if ($hasGeneralInstruction && trim((string) ($finalSuggested[$rateKey]['general_account'] ?? '')) !== $general) {
                $finalSuggested[$rateKey]['general_account'] = $general;
                $aiInstructionApplied++;
            }
            if ($hasIvaInstruction && normalizeRateKey($rateKey) !== '0' && trim((string) ($finalSuggested[$rateKey]['iva_account'] ?? '')) !== $iva) {
                $finalSuggested[$rateKey]['iva_account'] = $iva;
                $aiInstructionApplied++;
            }
        }
