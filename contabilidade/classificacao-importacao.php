            ...
            <?php foreach ($rows as $row): ?>
                <tr>
                    ...
                    <?php
                        ...
                        $accIva6 = $accounts['iva6'] ?? ($row['account_iva6'] ?? '');
                        $accIva13 = $accounts['iva13'] ?? ($row['account_iva13'] ?? '');
                        $accIva23 = $accounts['iva23'] ?? ($row['account_iva23'] ?? '');
                        $accNovat = $accounts['novat'] ?? ($row['account_novat'] ?? '');
                        $allAccounts = (
                            (!$hasIva6 || (int)$accIva6 > 0) &&
                            (!$hasIva13 || (int)$accIva13 > 0) &&
                            (!$hasIva23 || (int)$accIva23 > 0) &&
                            (!$needsNovat || (int)$accNovat > 0)
                        );
                        $btnClass = $allAccounts ? 'btn-success' : 'btn-warning';
                    ?>
                    <td class="text-center">
                        <button type="button" class="btn btn-xs <?= $btnClass; ?> classify-row" data-id="<?= (int)$row['id']; ?>" data-iva6="<?= htmlspecialchars($accIva6); ?>" data-iva13="<?= htmlspecialchars($accIva13); ?>" data-iva23="<?= htmlspecialchars($accIva23); ?>" data-novat="<?= htmlspecialchars($accNovat); ?>" data-req-iva6="<?= $hasIva6 ? 1 : 0; ?>" data-req-iva13="<?= $hasIva13 ? 1 : 0; ?>" data-req-iva23="<?= $hasIva23 ? 1 : 0; ?>" data-req-novat="<?= $needsNovat ? 1 : 0; ?>" data-emitter="<?= htmlspecialchars($row['field_A'] ?? ''); ?>" data-acquirer="<?= htmlspecialchars($row['field_B'] ?? ''); ?>" data-doctype="<?= htmlspecialchars($row['field_D'] ?? ''); ?>">Classificar</button>
