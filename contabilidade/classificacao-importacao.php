            ...
            <?php foreach ($rows as $row): ?>
                <tr>
                    ...
                    <td class="text-center">
                        <button type="button" class="btn btn-xs <?= $btnClass; ?> classify-row" data-id="<?= (int)$row['id']; ?>" data-iva6="<?= (int)($row['account_iva6'] ?? 0) > 0 ? htmlspecialchars($row['account_iva6']) : '' ?>" data-iva13="<?= (int)($row['account_iva13'] ?? 0) > 0 ? htmlspecialchars($row['account_iva13']) : '' ?>" data-iva23="<?= (int)($row['account_iva23'] ?? 0) > 0 ? htmlspecialchars($row['account_iva23']) : '' ?>" data-novat="<?= (int)($row['account_novat'] ?? 0) > 0 ? htmlspecialchars($row['account_novat']) : '' ?>" data-amt-iva6="<?= $amtIva6; ?>" data-amt-iva13="<?= $amtIva13; ?>" data-amt-iva23="<?= $amtIva23; ?>" data-emitter="<?= htmlspecialchars($row['field_A'] ?? ''); ?>" data-acquirer="<?= htmlspecialchars($row['field_B'] ?? ''); ?>" data-doctype="<?= htmlspecialchars($row['field_D'] ?? ''); ?>">Classificar</button>
